...
void randShot(char ground[][10], struct ship *list){
    int x, y;
    srand(time(NULL));
    x = rand() % 10;
    y = rand() % 10;
    while ((9 < x) || (x < 0) || (9 < y) || (y < 0) || ((ground[x][y] != 's') && (ground[x][y] != '~')))    {
        x = rand() % 10;
        y = rand() % 10;
    }
    ...
    if (ground[x][y] == 's')    {
        ...
        printf("to Expload...");
        getchar();
        expload(list, x, y, ground);
        enemyEZrinter(ground);
        getchar();
        randShot(ground, list);
    }
    if (ground[x][y] == '~')    {
        ground[x][y] = 'w';
        return;
